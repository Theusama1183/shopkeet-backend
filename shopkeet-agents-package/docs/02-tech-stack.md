...
| Hosting / infra | **Docker Compose + Caddy** on a single VPS (Hetzner, DigitalOcean), fronted by **Cloudflare** (proprietary — see note below) | Caddy's on-demand TLS auto-provisions a Let's Encrypt cert per domain (subdomains and merchant custom domains), gated by an `ask` endpoint so only real tenant domains get certs. Deploys are `scp`/`ssh` + `docker compose up -d`. Cloudflare handles DNS/edge caching/DDoS in front of `shopkeet.com` itself. |
