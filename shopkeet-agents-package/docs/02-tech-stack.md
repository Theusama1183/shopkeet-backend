...
| Hosting / infra | **Coolify** (open source, self-hosted PaaS) on a single VPS (Hetzner, DigitalOcean), fronted by **Cloudflare** (proprietary — see note below) | Coolify's built-in Traefik proxy auto-provisions TLS per domain (subdomains and merchant custom domains) with git-push deploys. Cloudflare handles DNS/edge caching/DDoS in front of `shopkeet.com` itself. |
