...
// Reserver claims unit "slots" when a variant is added to a cart. This is only
// a fast path: the authoritative anti-oversell guard is the checkout-time
// `SELECT ... FOR UPDATE` on product_variants (Phase 8). Therefore reservation
// is strictly best-effort and must never fail a cart operation — if Redis is
...
type Reserver interface {
	...
	// variant. Returns nil even on Redis errors (best-effort).
	ReserveUnits(ctx context.Context, variantID string, quantity int) error
	Close() error
}
...
// RedisReserver SETNXes shopkeet:reserve:{variant_id}:{unit_index} for each
...
func (r *RedisReserver) ReserveUnits(ctx context.Context, variantID string, quantity int) error {
	if r == nil || r.rdb == nil || quantity <= 0 {
		return nil
	}
	for i := 0; i < quantity; i++ {
		key := fmt.Sprintf("shopkeet:reserve:%s:%d", variantID, i)
